...
$course=$db->query("SELECT * from practice WHERE idCourse='$course_id' AND number='$exercise'");
$course=$course->fetch(PDO::FETCH_ASSOC);

if(isset($_POST['task']) && isset($_POST['answer'])){

    if($_POST['task']!=$course['description']){
        $db->query("UPDATE practice SET description='${_POST['task']}' WHERE idCourse='$course_id' AND number='$exercise'");
    }

    if($_POST['answer']!=$course['answer']){
        $db->query("UPDATE practice SET answer='${_POST['answer']}' WHERE idCourse='$course_id' AND number='$exercise'");
    }
    ...
    elseif (isset($_POST['submitEx'])){
        $exercise++;
        $exercises=$db->query("SELECT * FROM practice WHERE idCourse='$course_id'");
        if($exercises->rowCount()>$exercise) {
            header("Location: update_exercises.php?course_id=$course_id&exercise=$exercise");
        }
        else{
    ...
    <div style="font-size: 20px">
        <form method="post">
            <fieldset style="border: 2px dashed crimson; width: fit-content; height: fit-content; padding: 15px">
                Task description:<input type="text" name="task" value="<?=$course['description']?>" placeholder=" volume of exercise..." required><br>
                Answer: <input type="text" name="answer" value="<?=$course['answer']?>" placeholder=" provide with link..."><br>
                Videos(optional):
                ...
                <button type="submit" name="submit" class="inline-option-button">UPDATE</button>  OR   <button type="submit" name="submitEx" class="inline-option-button">UPDATE NEXT TASK</button>
            </fieldset>
        </form>
    </div>
</section>
...
<script>
    ...
    function getVideos() {
        ...
        console.log(videos); // You can perform further processing with the videos array
    }
    ...
    function getImages() {
        ...
        console.log(images); // You can perform further processing with the videos array
    }
</script>
